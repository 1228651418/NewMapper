...
import com.jyx.edu.NewMapper180831.pojo.PageTreeView;
...
public interface BaseService<T>{
    ...
    int insert(T record, SysUser operator);//保存一个实体，null的属性也会保存，不会使用数据库默认值

    int insertSelective(T record,SysUser operator);//保存一个实体，null的属性不会保存，会使用数据库默认值

    int insertUseGeneratedKeys(T record, SysUser operator);//回填id到对象，Mysql专用,MySqlMapper<T>

    int insertList(List<T> recordList, SysUser operator);//Mysql专用,MySqlMapper<T>

    int updateByPrimaryKey(T record,SysUser operator);//根据主键更新实体全部字段，null值会被更新

    int updateByPrimaryKeySelective(T record,SysUser operator);//根据主键更新属性不为null的值

    int updateByExample(@Param("record") T record, @Param("example") Object example, SysUser operator);//根据Example条件更新实体record包含的全部属性，null值会被更新

    int updateByExampleSelective(@Param("record") T record, @Param("example") Object example,SysUser operator);//根据Example条件更新实体record包含的不是null的属性值
